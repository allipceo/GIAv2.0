...
# -*- coding: utf-8 -*-
"""
한진중공업 완전한 뉴스 클리핑 노션 업로드 스크립트
작성일: 2025년 8월 6일
작성자: 서대리 (Lead Developer)
목적: 실제 뉴스 제목 + 내용 + 링크를 포함한 완전한 뉴스 클리핑 업로드
"""
...
def load_enhanced_news_data(filename="../data/hanjin_enhanced_news_data.json"):
    """향상된 뉴스 데이터 로드"""
    try:
...
def upload_enhanced_news_to_notion(article):
    """완전한 뉴스 클리핑을 노션 DB에 업로드"""
...
def main():
    """메인 실행 함수"""
    print("🚀 한진중공업 완전한 뉴스 클리핑 업로드 시작...")
    
    # 1. 향상된 뉴스 데이터 로드
    articles = load_enhanced_news_data()
    if not articles:
        print("❌ 업로드할 뉴스 데이터가 없습니다.")
        print("💡 먼저 hanjin_news_collector_enhanced.py를 실행하여 뉴스를 수집하세요.")
        return
    ...
    for article in articles:
        ...
        # 업로드 실행
        if upload_enhanced_news_to_notion(article):
            success_count += 1

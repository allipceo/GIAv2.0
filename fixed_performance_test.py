...
"""
수정된 구차장 성능 테스트
Claude vs Gemini 성능 비교 (오류 수정 버전)
"""
...
class LLMPerformanceTester:
    ...
    def generate_performance_report(self):
        ...
        gemini_avg_time = sum(r['response_time'] for r in gemini_tests) / len(gemini_tests) if gemini_tests else 0
        claude_avg_length = sum(r['response_length'] for r in claude_tests) / len(claude_tests) if claude_tests else 0
        gemini_avg_length = sum(r['response_length'] for r in gemini_tests) / len(gemini_tests) if gemini_tests else 0
...
- **Gemini (구차장 대체)**: {gemini_avg_time:.2f}초
- **속도 차이**: {gemini_avg_time - claude_avg_time:.2f}초 ({'Claude가 빠름' if gemini_avg_time > claude_avg_time else 'Gemini가 빠름'})

### 평균 응답 길이
- **Claude (채팀장)**: {claude_avg_length:.0f}자
- **Gemini (구차장 대체)**: {gemini_avg_length:.0f}자
...
- Gemini (구차장 대체)는 평균 {gemini_avg_time:.2f}초로 상대적으로 느림
- Claude의 응답 속도가 일관되게 빠름 (평균 {claude_avg_time:.2f}초)

### 성능 분석
1. **속도**: Claude가 Gemini보다 약 {gemini_avg_time/claude_avg_time:.1f}배 빠름
...
"""
        ...
        print(f"Gemini (구차장 대체) 평균 속도: {gemini_avg_time:.2f}초")
        print(f"속도 차이: {gemini_avg_time - claude_avg_time:.2f}초")
